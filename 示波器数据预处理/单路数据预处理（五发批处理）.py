...
# 输入和输出文件夹路径
folder_path = r'D:\03  实验数据\绝缘件对等离子体参数的影响\绝缘件二\单路\光电流'
output_folder = r'D:\03  实验数据\绝缘件对等离子体参数的影响\绝缘件二\单路\光电流\数据预处理'
...
# 定义时间偏移量和发射次数
time_offsets = [0, 20000, 40000, 60000, 80000]
num_shots = 5
    ...
    with open(os.path.join(folder_path, csv_file), 'r') as input_file:
        ...
        for row in csv_reader:
            ...
            for i, offset in enumerate(time_offsets):
                if (i * 20000 -500) <= new_row[0] <= (i * 20000 + 2000 ):
                    new_row[0] -= offset
